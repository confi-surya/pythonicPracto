...
TEST(listContianerTest, listingContainer)
{
	...
	boost::shared_ptr<accountInfoFile::AccountInfoFileManager> a_file_man(new accountInfoFile::AccountInfoFileManager());
        ...
        boost::shared_ptr<recordStructure::ContainerRecord> container_record_3(
					new recordStructure::ContainerRecord(100, "my_container_deleted", 1000, 2000, 3000, 4000, true));
...
//	for(; it != container_records.end(); it++)
//		std::cout<<"ou "<<it->get_name()<< " "<<it->get_ROWID()<<" "<<it->get_deleted()<<std::endl;
