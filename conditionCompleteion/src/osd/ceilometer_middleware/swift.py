...
class ObjectsPollster(_Base):
    ...
    def get_samples(self, manager, cache, resources=[]):
        for tenant, account in self._iter_accounts(manager.keystone, cache):
            ...
            container_info = account[1]
            for container in container_info:
                if container['name'] == '$Policies$':
                    account_data['x-account-object-count'] = str(int(account_data['x-account-object-count']) - int(container['count']))
            yield sample.Sample(
                name='storage.objects',
                type=sample.TYPE_GAUGE,
                volume=int(account_data['x-account-object-count']),
                unit='object',
                user_id=None,
                project_id=tenant,
                resource_id=tenant,
...
class ObjectsSizePollster(_Base):
    ...
    def get_samples(self, manager, cache, resources=[]):
        for tenant, account in self._iter_accounts(manager.keystone, cache):
            ...
            for container in container_info:
                if container['name'] == '$Policies$':
                    account_data['x-account-bytes-used'] = str(int(account_data['x-account-bytes-used']) - int(container['bytes']))
            yield sample.Sample(
                name='storage.objects.size',
                type=sample.TYPE_GAUGE,
                volume=int(account_data['x-account-bytes-used']),
                unit='B',
                user_id=None,
                project_id=tenant,
                resource_id=tenant,
...
class ObjectsContainersPollster(_Base):
    ...
    def get_samples(self, manager, cache, resources=[]):
        for tenant, account in self._iter_accounts(manager.keystone, cache):
            ...
            for container in container_info:
                if container['name'] == '$Policies$':
                    account_data['x-account-container-count'] = str(int(account_data['x-account-container-count']) - 1)
            yield sample.Sample(
                name='storage.objects.containers',
                type=sample.TYPE_GAUGE,
                volume=int(account_data['x-account-container-count']),
                unit='container',
                user_id=None,
                project_id=tenant,
                resource_id=tenant,
    ...
    def get_samples(self, manager, cache, resources=[]):
        for project, account in self._iter_accounts(manager.keystone, cache):
            ...
            for container in containers_info:
                LOG.info('object polster %s '%container)
                if container['name'] == "$Policies$":
                    continue
                yield sample.Sample(
...
class ContainersSizePollster(_Base):
    ...
    def get_samples(self, manager, cache, resources=[]):
        for project, account in self._iter_accounts(manager.keystone, cache):
            containers_info = account[1]
            for container in containers_info:
                LOG.info('container size polster %s '%container)
                if container['name'] == "$Policies$":
                    continue
                yield sample.Sample(
