...
from osd.common.utils import ContextPool, normalize_timestamp, \
    config_true_value, public, json, csv_append, GreenthreadSafeIterator, \
    quorum_size, GreenAsyncPile, normalize_delete_at_timestamp, loggit
from osd.common.bufferedhttp import http_connect
from osd.common.constraints import check_metadata, check_object_creation, \
...
from osd.common.http import is_success, is_client_error, HTTP_CONTINUE, \
...
from osd.common.swob import HTTPAccepted, HTTPBadRequest, HTTPNotFound, \
    HTTPPreconditionFailed, HTTPRequestEntityTooLarge, HTTPRequestTimeout, \
    HTTPServerError, HTTPServiceUnavailable, Request, HTTPConflict, \
    HTTPClientDisconnect, HTTPNotImplemented, HTTPInternalServerError
from osd.common.request_helpers import is_user_meta, get_param
...
class ObjectController(Controller):
    ...
    def GETorHEAD(self, req):
        ...
        container_info, head_status = self.container_info(
        ...
        if 'swift.authorize' in req.environ:
            ...
            if aresp:
                return aresp
        if not container_info['node'] and not head_status:
        ...
        if head_status and int(str(head_status).split()[0]) == 503:
            ...
            self.app.logger.info("container HEAD returning 503 service" \
                "unavailable error due to which this request got failed")
            return HTTPServiceUnavailable(request=req)
        if head_status and int(str(head_status).split()[0]) == 500:
            return HTTPInternalServerError(request=req)

        if not container_info['node']:
            return HTTPNotFound(request=req)
        container, container_filesystem, container_directory, \
            global_map_version, component_number = \
            self.app.container_ring.get_node(self.account_name, self.container_name)
        if not len(container):
            self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                {'msg': _('ERROR Wrong ring file content'),
                'method': 'GET/HEAD', 'path': req.path})
            return HTTPInternalServerError(request=req)
        req.headers['X-Container-Filesystem'] = container_filesystem
        req.headers['X-Container-Directory'] = container_directory
        req.headers['X-Container-Host'] = '%(ip)s:%(port)s' % container[0]

        node, filesystem, directory, global_map_version, obj_comp_number = \
            self.app.object_ring.get_node(self.account_name, \
...
#            req.swift_entity_path)
        resp = self.GETorHEAD_base(
            req, _('Object'), self.app.object_ring, node, filesystem, directory,
...
#            container_partition = container_info['partition']
            container_filesystem = container_info['filesystem']
            container_directory = container_info['directory']
#            containers = container_info['nodes']
            container = container_info['node']
            container_component_number = container_info['component_number']
            req.acl = container_info['write_acl']
            if 'swift.authorize' in req.environ:
                aresp = req.environ['swift.authorize'](req)
                if aresp:
                    return aresp
            if not container and not head_status:
                return HTTPInternalServerError(request=req)
            if head_status and int(str(head_status).split()[0]) == 503:
                #TODO need to check why head_status is int or sometimes str
                self.app.logger.info("container HEAD returning 503 service" \
...
#                delete_at_container, delete_at_part, delete_at_nodes)
            try:
                headers = self._backend_requests(
                    req, len(node), container_filesystem, container_directory,
                    container, global_map_version, container_component_number)
...
#                                      'POST', req.swift_entity_path, headers)
            resp = self.make_requests(req, self.app.object_ring, node,
                                  filesystem, directory, 'POST',
...
#                          delete_at_nodes=None):
    def _backend_requests(self, req, n_outgoing, container_filesystem,
        ...
        for i, container in enumerate(containers):
            try:
                ...
                raise
            headers[i]['X-Container-Filesystem'] = container_filesystem
            headers[i]['X-Container-Directory'] = container_directory
            headers[i]['X-Container-Host'] = '%(ip)s:%(port)s' % container
...
#                          logger_thread_locals):
    def _connect_put_node(self, nodes, filesystem, directory, path, headers,
                          logger_thread_locals):
...
#                        path, headers)
                    conn = http_connect(
                        node['ip'], node['port'], filesystem, directory, 'PUT',
                        path, headers)
...
#        if get_param(req, 'multipart-manifest'):
#            return HTTPBadRequest(request=req, content_type='text/plain',
#                                  body='Unsupported query parameter '
#                                       'multipart manifest')
        self.app.logger.debug("Receives PUT request")
        container_info, head_status = self.container_info(
            self.account_name, self.container_name, req)
#        container_partition = container_info['partition']
#        containers = container_info['nodes']
        self.app.logger.debug("PUT request container_info:%s" %container_info)
        container_fs = container_info['filesystem']
        container_dir = container_info['directory']
        container_node = container_info['node']
        container_component_number = container_info['component_number']
        req.acl = container_info['write_acl']
        ...
        if 'swift.authorize' in req.environ:
            ...
            if aresp:
                return aresp
        if not container_node and not head_status:
            return HTTPInternalServerError(request=req)
        if head_status and int(str(head_status).split()[0]) == 503:
            #TODO need to check why head_status is int or sometimes str
            self.app.logger.info("container HEAD returning 503 service" \
...
#            self.account_name, self.container_name, self.object_name)
        ...
        if 'x-timestamp' in req.headers or \
            ...
            hresp = self.GETorHEAD_base(
                hreq, _('Object'), self.app.object_ring, node, filesystem, directory,
                hreq.swift_entity_path, global_map_version, container_component_number)
        ...
        source_resp = None
        if source_header:
            if req.environ.get('swify.orig_req_method', req.method) != 'POST':
                req.environ.setdefault('swift.log_info', []).append(
...
#            self.iter_nodes_local_first(self.app.object_ring, partition))
        ...
        node_iter = GreenthreadSafeIterator(node)
#        pile = GreenPile(len(nodes))
        pile = GreenPile(len(node))
...
#            delete_at_container, delete_at_part, delete_at_nodes)
        try:
            outgoing_headers = self._backend_requests(
                req, len(node), container_fs, container_dir, container_node,
                global_map_version, container_component_number)
        except ZeroDivisionError:
            self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                {'msg': _('ERROR Wrong ring file content'),
                'method': 'PUT', 'path': req.path})
...
#                       self.app.logger.thread_locals)
            pile.spawn(self._connect_put_node, node_iter, filesystem,
                       directory, req.swift_entity_path, nheaders,
                       self.app.logger.thread_locals)

        conns = [conn for conn in pile if conn]
        min_conns = quorum_size(len(node))
...
#            with ContextPool(len(nodes)) as pool:
            with ContextPool(len(node)) as pool:
                for conn in conns:
                    conn.failed = False
            ...
            self.app.logger.warn(
                _('ERROR Object write timeout (%ss)'), err.seconds)
            self.app.logger.increment('client_timeouts')
            conn.close()
            return HTTPRequestTimeout(request=req)
        except (Exception, Timeout):
...
#                                  headers)
        try:
            headers = self._backend_requests(req, len(node),
                container_filesystem, container_directory, container,
                global_map_version, container_component_number)
        except ZeroDivisionError:
            self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                {'msg': _('ERROR Wrong ring file content'),
            ...
            return HTTPInternalServerError(request=req)
        resp = self.make_requests(req, self.app.object_ring, node,
                                  filesystem, directory, 'DELETE', req.swift_entity_path,
                                  headers)
        return resp
