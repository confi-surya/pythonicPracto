...
from osd.common.utils import normalize_timestamp, config_true_value, \
    public, list_from_csv, GreenthreadSafeIterator, \
    quorum_size, GreenAsyncPile, loggit, float_comp
from osd.common.helper_functions import split_path
from osd.common.bufferedhttp import http_connect
from osd.common.exceptions import ChunkReadTimeout, ChunkWriteTimeout, \
    ConnectionTimeout, GenericTimeout, UpdateGlobalMapTimeout
from osd.common.http import is_informational, is_success, is_redirection, \
    is_server_error, HTTP_OK, HTTP_PARTIAL_CONTENT, HTTP_MULTIPLE_CHOICES, \
    HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_SERVICE_UNAVAILABLE, \
    HTTP_INSUFFICIENT_STORAGE, HTTP_UNAUTHORIZED
from osd.common.swob import Request, Response, HeaderKeyDict, Range, \
    HTTPException, HTTPRequestedRangeNotSatisfiable, HTTPInternalServerError, \
    HTTPNotFound, HTTPForbidden, HTTPServiceUnavailable
from osd.common.request_helpers import strip_sys_meta_prefix, \
...
def get_container_info(env, app, swift_source=None):
    ...
    info, head_status = get_info(app, env, account, container, \
        ret_not_found=True, swift_source=swift_source)
    if not info:
...
def get_account_info(env, app, swift_source=None):
    ...
    info, head_status = get_info(app, env, account, ret_not_found=True,
                    swift_source=swift_source)
    if not info:
...
def get_info(app, env, account, container=None, ret_not_found=False,
    ...
    if container:
        ...
        info, status = get_info(app, env, account)
        if not info:
            return None, status
        path += '/' + container
...
#                 backend_headers):
    def __init__(self, app, req, server_type, ring, node, filesystem,
                 directory, path, backend_headers):
        self.app = app
        self.ring = ring
            ...
            while True:
                ...
                if not chunk:
                    if (content_length - bytes_read_from_source) > 0:
                        self.app.logger.error(_('Incomplete bytes'))
                        raise
                    break
...
#                        query_string=self.req_query_string)
                    conn = http_connect(
                        node['ip'], node['port'], self.filesystem,
                        self.directory, self.req_method, self.path,
                        headers=self.backend_headers,
                        query_string=self.req_query_string)
...
#                    self.app.error_limit(node, _('ERROR Insufficient Storage'))
                    self.app.logger.error(_('%(msg)s %(ip)s:%(port)s'),
                          {'msg': _('ERROR Insufficient Storage'), 'ip': node['ip'],
                          'port': node['port']})
...
#                         'type': self.server_type})
                    self.app.logger.error(_('%(msg)s %(ip)s:%(port)s'),
                          {'msg': _('ERROR %(status)d %(body)s '
                                    'From %(type)s Server') %
                          {'status': possible_source.status,
                           'body': self.bodies[-1][:1024],
                           'type': self.server_type}, 'ip': node['ip'],
                          'port': node['port']})
...
class Controller(object):
    ...
    def generate_request_headers(self, global_map_version, component_number,
                                 orig_req=None, additional=None, 
                                 transfer=False):
        """
        ...
        headers['x-component-number'] = component_number
        return headers
...
#        partition, nodes = self.app.account_ring.get_nodes(account)
        if req:
            ...
            env = {}
        info, head_status = get_info(self.app, env, account)
        if not info:
            return None, None, None, None, None, head_status
        if info.get('container_count') is None:
            container_count = 0
        else:
...
#        return partition, nodes, container_count
        node, filesystem, directory, global_map_version, component_number = \
            self.app.account_ring.get_node(account)
        if not node:
            return None, None, None, None, None, None
        return node, filesystem, directory, container_count, \
            component_number, head_status
...
#        part, nodes = self.app.container_ring.get_nodes(account, container)
        if req:
            ...
            env = {}
        info, head_status = get_info(self.app, env, account, container)
        if not info:
            info = headers_to_container_info({}, 0)
#            info['partition'] = None
#
#            info['nodes'] = None
            ...
            info['directory'] = None
            info['component_number'] = None
        else:
            service_details, filesystem, directory, global_map_version, \
                component_number = self.app.container_ring.\
                get_node(account, container)
...
#            info['nodes'] = nodes
                ...
                info['node'] = service_details
                info['component_number'] = component_number
        return info, head_status

#    def _make_request(self, nodes, part, method, path, headers, query,
#                      logger_thread_locals):
    def _make_request(self, nodes, filesystem, directory, method, path,
                      headers, query, logger_thread_locals):
        """
...
#                      query_string=''):
    def make_requests(self, req, ring, node, filesystem, directory, method,
                      path, headers, query_string=''):
        """
...
#                       head, query_string, self.app.logger.thread_locals)
            pile.spawn(self._make_request, nodes, filesystem, directory, method, path,
                       head, query_string, self.app.logger.thread_locals)
        response = []
        statuses = []
        for resp in pile:
...
#        partition, nodes = self.app.account_ring.get_nodes(account)
        node, filesystem, directory, global_map_version, component_number = \
            self.app.account_ring.get_node(account)
        if not len(node):
            self.app.logger.warning('Could not autocreate account %s' % account)
            self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                {'msg': _('ERROR Wrong ring file content'),
                'method': 'PUT', 'path': account})
            return HTTPInternalServerError()
        path = '/%s' % account
        headers = {'X-Timestamp': normalize_timestamp(time.time()),
                   'X-Trans-Id': self.trans_id,
                   'Connection': 'close',
                   'x-global-map-version': global_map_version,
                   'x-component-number': component_number}
#        resp = self.make_requests(Request.blank('/v1' + path),
#                                  self.app.account_ring, partition, 'PUT',
#
#                                  path, [headers] * len(nodes))
        resp = self.make_requests(Request.blank('/v1' + path),
                                  self.app.account_ring, node,
                                  filesystem, directory, 'PUT',
                                  path, [headers] * len(node))
        try:
            #Global map is not updated within node_timeout
            with UpdateGlobalMapTimeout(self.app.node_timeout):
                #Update global map in case of response status is 301 or 
                #307 or 503
                retry = 3
                counter = 0
                while int(resp.status.split()[0]) in (301, 307, 503) and \
                    counter < retry:
                    counter += 1
                    self.app.logger.warning("autocreate_account %r status:%s, "\
                        "therefore sending global map update request, retry:%s"\
                        %(path, resp.status, counter))
                    self.app.account_ring.ring_data.update_global_map()
                    node, filesystem, directory, global_map_version, \
                        component_number = self.app.account_ring.get_node(account)
                    if not len(node):
                        self.app.logger.warning('Could not autocreate account %s' % account)
                        self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                            {'msg': _('ERROR Wrong ring file content'),
                    ...
                    resp = self.make_requests(Request.blank('/v1' + path),
                                  ...
                                  filesystem, directory, 'PUT',
                                  path, [headers] * len(node))
                    if int(resp.status.split()[0]) in (301, 307, 503) and counter < retry:
                        sleep(self.app.request_retry_time_service_unavailable)
                if int(resp.status.split()[0]) in (301, 307):
                    resp = HTTPInternalServerError()
        except UpdateGlobalMapTimeout:
            resp = self.update_map_timeout_exception(_('Account'), '', component_number)
...
#    def GETorHEAD_base(self, req, server_type, ring, partition, path):
    def GETorHEAD_base(self, req, server_type, ring, node,
                       filesystem, directory, path, global_map_version, 
                       component_number):
        """
        ...
        """
        backend_headers = self.generate_request_headers(global_map_version, \
            component_number, req, additional=req.headers)

#        handler = GetOrHeadHandler(self.app, req, self.server_type, ring,
#
#                                   partition, path, backend_headers)
        handler = GetOrHeadHandler(self.app, req, self.server_type, ring,
                                   node, filesystem, directory, path,
                                   backend_headers)
    ...
    def add_component_in_block_req(self, server_type, component_number):
        """
        Add component in block request to prevent map update request 
        for same component request

        :param server_type: request for server(Account, Container)
        :param component_number: component_number of request
        """
        self.block_req_dict[server_type, component_number] = True

    def delete_component_from_block_req(self, server_type, component_number):
        """
        Delete component in block request after map update request
        for further request processing

        :param server_type: request for server(Account, Container)
        :param component_number: component_number of request
        """
        del self.block_req_dict[server_type, component_number]

    def is_req_blocked(self, server_type, component_number):
        """
        To check if request is blocked for component(as component movement can
        be in progress or complete or component owner service is unavailable )

        :param server_type: request for server(Account, Container)
        :param component_number: component_number of request
        """
        if not self.block_req_dict:
            return False
        for block_comp, value in self.block_req_dict.items():
            #if request for component is blocked then sleep and again check if
            #request is blocked or not and repeat until request is unblocked
            if block_comp == (server_type, component_number):
                self.app.logger.warning("Request of:%s service for component :%s"\
                    " is blocked as component movement is in progress " \
                    %(server_type, component_number))
                sleep(10)		#TODO need to check for this timeout
                self.is_req_blocked(server_type, component_number)
        else:
            return False

    def update_map_timeout_exception(self, server_type, req, component_number):
        self.app.logger.error("Timeout occured during global map update")
        if server_type == 'Account':
            if self.app.account_ring.ring_data._update_map_request:
                self.app.account_ring.ring_data._update_map_request = False
                if self.app.account_ring.ring_data.conn_obj:
                    self.app.account_ring.ring_data.conn_obj.close()
        elif server_type == 'Container':
            if self.app.container_ring.ring_data._update_map_request:
                self.app.container_ring.ring_data._update_map_request = False
                if self.app.container_ring.ring_data.conn_obj:
                    self.app.container_ring.ring_data.conn_obj.close()
        for block_comp, value in self.block_req_dict.items():
            if block_comp == (server_type, component_number):
                del self.block_req_dict[server_type, component_number]
        return HTTPInternalServerError(request=req)

    def account_req_for_comp_distribution(self, req, component_number, \
                       global_map_version, account_name, resp):
        counter = 0
        counter_service_unavailable = 0
        try:
            #Global map is not updated within node_timeout
            with UpdateGlobalMapTimeout(self.app.node_timeout):
                #Update global map in case of response status is 301 or 307
                #or 503 and retries is less than 3
                while int(resp.status.split()[0]) in (301, 307) \
                    and counter < self.app.retry_count:
                    counter += 1
                    self.app.logger.warning("%s method, intermediate " \
                        "response: %s, transaction-id: %s, retry:%s" %(\
                        req.method, resp.status, req.headers.get\
                        ('x-trans-id', '-'), counter))
                    resp, global_map_version = self.update_map_for_account(\
                        req, component_number, global_map_version, \
                        account_name, resp)
                while int(resp.status.split()[0]) == 503 and \
                    counter_service_unavailable < self.app.retry_count:
                    counter_service_unavailable += 1
                    self.app.logger.warning("%s method, intermediate " \
                        "response: %s, transaction-id: %s, retry:%s" %(\
                        req.method, resp.status, req.headers.get\
                        ('x-trans-id', '-'), counter_service_unavailable))
                    self.app.logger.debug("Updating global map for account: %s "\
                        "old global_map_version:%s" %(req.method, global_map_version))
                    self.add_component_in_block_req(_('Account'), component_number)
                    self.app.account_ring.ring_data.update_global_map()
                    self.app.logger.warning("old_map_version:%s, current_map_"
                        "version:%s" %(global_map_version, self.app.\
                         account_ring.ring_data.get_global_map_version()))

                    resp, global_map_version = self.retry_request_of_account(\
                        req, component_number, global_map_version, \
                        account_name, resp)
                    #if response status is 503 i.e service unavailable(in 
                    #case of recovery on same node or other node) then 
                    #sleep 100 seconds and send global map update request 
                    #once and process request further
                    if int(resp.status.split()[0]) == 503 and \
                        counter_service_unavailable < self.app.retry_count:
                        sleep(self.app.request_retry_time_service_unavailable)  #TODO need to check sleep value
        except UpdateGlobalMapTimeout:
            #if node_timeout occured during global map update requests then
            #close communication connection object and set update_map_request
            #to false so that other requests can send map update request
            resp = self.update_map_timeout_exception(_('Account'), req, component_number)
        return resp

    def update_map_for_account(self, req, component_number, \
                            global_map_version, account_name, resp):
        self.app.logger.debug("Updating global map for account: %s "\
            "old global_map_version:%s" %(req.method, global_map_version))
        self.add_component_in_block_req(_('Account'), component_number)
        ...
        self.app.logger.warning("old_map_version:%s, current_map_"
            "version:%s" %(global_map_version, self.app.\
            account_ring.ring_data.get_global_map_version()))
        #if response status is in (301, 307) then send global map
        #update request until map version is not updated and 
        #retry is less than 3
        while float_comp(global_map_version, self.app.\
            account_ring.ring_data.get_global_map_version()) == 0 \
            and counter < self.app.retry_count:
            counter += 1
            sleep(self.app.request_retry_time_component_transfer)           #TODO need to check sleep value
            self.app.account_ring.ring_data.update_global_map()
            self.app.logger.debug("old_map_version:%s, current_map"
                "_version:%s, retry:%s" %(global_map_version, self.app.\
                account_ring.ring_data.get_global_map_version(), counter))
        #if global map is not updated after 3 retires(150 seconds) then
        #send previous response to account controller 
        if counter == 3 and float_comp(global_map_version, self.app.\
            account_ring.ring_data.get_global_map_version()) == 0:
            return resp, global_map_version
        while float_comp(global_map_version, self.app.\
            account_ring.ring_data.get_global_map_version()) < 0 \
            and counter < self.app.retry_count:
            counter += 1
            ...
            self.app.logger.debug("old_map_version:%s, current_map"
                "_version:%s, retry:%s" %(global_map_version, self.app.\
                account_ring.ring_data.get_global_map_version(), counter))
            resp, global_map_version = self.retry_request_of_account(req, \
                component_number, global_map_version, account_name, resp)
            if int(resp.status.split()[0]) not in (301, 307) or counter == \
                self.app.retry_count:
                return resp, global_map_version
            self.add_component_in_block_req(_('Account'), component_number)
            sleep(self.app.request_retry_time_component_transfer)
            self.app.account_ring.ring_data.update_global_map()
        return self.retry_request_of_account(req, component_number, \
            global_map_version, account_name, resp)


    def retry_request_of_account(self, req, component_number, \
                            global_map_version, account_name, resp):
        self.delete_component_from_block_req(_('Account'), component_number)

        node, filesystem, directory, global_map_version, component_number = \
                self.app.account_ring.get_node(account_name)
        if not len(node):
            self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                              {'msg': _('ERROR Wrong ring file content'),
                              'method': req.method, 'path': req.path})  
            return HTTPInternalServerError(request=req), global_map_version

        if req.method in ('GET', 'HEAD'):
            resp = self.GETorHEAD_base(req, _('Account'), \
                       self.app.account_ring, node, filesystem, \
        ...
        clear_info_cache(self.app, req.environ, account_name)
        resp = self.make_requests(
            req, self.app.account_ring, node, filesystem, directory, req.method,
            req.swift_entity_path, [headers] * len(node))
        return resp, global_map_version

    def container_req_for_comp_distribution(self, req, component_number, \
                    global_map_version, account_name, container_name, resp):
        counter = 0
        try:
            #Global map is not updated within node_timeout
            with UpdateGlobalMapTimeout(self.app.node_timeout):
                ...
                while int(resp.status.split()[0]) in (301, 307) \
                    ...
                    self.app.logger.warning("%s method, intermediate " \
                        ...
                        req.method, resp.status, req.headers.get\
                        ('x-trans-id', '-'), counter))
                    resp, global_map_version = self.update_map_for_container(\
                        req, component_number, global_map_version, \
                        account_name, container_name, resp)
                while int(resp.status.split()[0]) == 503 and counter < \
                    self.app.retry_count:
                    counter += 1
                    self.app.logger.warning("%s method, intermediate " \
                        "response: %s, transaction-id: %s, retry:%s" %(\
                        req.method, resp.status, req.headers.get\
                        ('x-trans-id', '-'), counter))
                    self.app.logger.debug("Updating global map for container: %s, "\
                        "old global_map_version:%s" %(req.method, global_map_version))
                    self.add_component_in_block_req(_('Container'), component_number)
                    self.app.container_ring.ring_data.update_global_map()
                    self.app.logger.warning("old_map_version:%s, current_map_"
                        "version:%s" %(global_map_version, self.app.\
                        container_ring.ring_data.get_global_map_version()))

                    resp, global_map_version = self.retry_request_of_container(\
                        req, component_number, global_map_version, \
                        account_name, container_name, resp)
                    #if response status is 503 i.e service unavailable(in 
                    #case of recovery on same node or other node) then 
                    #sleep 100 seconds and send global map update request 
                    #once and process request further
                    if int(resp.status.split()[0]) == 503 and counter < \
                        self.app.retry_count:
                        sleep(self.app.request_retry_time_service_unavailable)  #TODO need to check sleep value
        except UpdateGlobalMapTimeout:
            #if node_timeout occured during global map update requests then
            #close communication connection object and set update_map_request
            #to false so that other requests can send map update request
            resp = self.update_map_timeout_exception(_('Container'), req, component_number)
        return resp

    def update_map_for_container(self, req, component_number, \
                    global_map_version, account_name, container_name, resp):
        self.app.logger.debug("Updating global map for container: %s, "\
            "old global_map_version:%s" %(req.method, global_map_version))
        self.add_component_in_block_req(_('Container'), component_number)
        ...
        #retry is less than 3
        while float_comp(global_map_version, self.app.\
            container_ring.ring_data.get_global_map_version()) == 0\
            and counter < self.app.retry_count:
            counter += 1
            sleep(self.app.request_retry_time_component_transfer)               #TODO need to check sleep value
            self.app.container_ring.ring_data.update_global_map()
            self.app.logger.debug("old_map_version:%s, current_map"
                "_version:%s, retry:%s" %(global_map_version, self.app.\
                container_ring.ring_data.get_global_map_version(), counter))
        #if global map is not updated after 3 retires(150 seconds) then
        #send previous response to account controller 
        if counter == 3 and float_comp(global_map_version, self.app.\
            container_ring.ring_data.get_global_map_version()) == 0:
        ...
        while float_comp(global_map_version, self.app.\
            ...
            self.add_component_in_block_req(_('Container'), component_number)
            self.app.logger.warning("old_map_version:%s, current_map_"
                "version:%s, retry:%s" %(global_map_version, self.app.\
                container_ring.ring_data.get_global_map_version(), counter))
            resp, global_map_version = self.retry_request_of_container(req, \
                component_number, global_map_version, account_name, \
                container_name, resp)
            if int(resp.status.split()[0]) not in (301, 307) or counter == \
                self.app.retry_count:
                return resp, global_map_version
            ...
            sleep(self.app.request_retry_time_component_transfer)
            self.app.container_ring.ring_data.update_global_map()
        return self.retry_request_of_container(req, component_number, \
            global_map_version, account_name, container_name, resp)


    def retry_request_of_container(self, req, component_number, \
                    global_map_version, account_name, container_name, resp):
        self.delete_component_from_block_req(_('Container'), component_number)

        if req.method in ('GET', 'HEAD'):
            node, filesystem, directory, global_map_version, component_number =\
                self.app.container_ring.get_node(account_name, container_name)
            if not len(node):
                self.app.logger.error(_('%(msg)s %(method)s %(path)s'),
                    {'msg': _('ERROR Wrong ring file content'),
                    'method': req.method, 'path': req.path})
                return HTTPInternalServerError(request=req), global_map_version
            resp = self.GETorHEAD_base(req, _('Container'), self.app.container_ring, \
        ...
        if head_status and int(str(head_status).split()[0]) == 503:
            ...
            return HTTPServiceUnavailable(request=req), global_map_version
        if head_status and int(str(head_status).split()[0]) == 500:
            return HTTPInternalServerError(request=req), global_map_version
        if req.method == 'PUT':
            if not account_node and self.app.account_autocreate:
                self.autocreate_account(req.environ, account_name)
                account_node, account_filesystem, account_directory, \
                container_count, account_component_number, head_status = \
                self.account_info(account_name, req)
            if not account_node and not head_status:
                return HTTPInternalServerError(request=req), global_map_version
            if head_status and int(str(head_status).split()[0]) == 503:
                self.app.logger.info("account HEAD returning 503 service" \
                    "available error due to which this request got failed")
                return HTTPServiceUnavailable(request=req), global_map_version
            if head_status and int(str(head_status).split()[0]) == 500:
                return HTTPInternalServerError(request=req), global_map_version
        if not account_node:
            return HTTPNotFound(request=req), global_map_version

        if req.method == 'PUT':
            if self.app.max_containers_per_account > 0 and \
                   container_count >= self.app.max_containers_per_account and \
                   account_name not in self.app.max_containers_whitelist:
        ...
        clear_info_cache(self.app, req.environ, account_name, container_name)
        resp = self.make_requests(
            req, self.app.container_ring,
            container_node, container_filesystem, container_directory,
            req.method, req.swift_entity_path, headers)
        return resp, global_map_version
