...
from osd.proxyService.unitTests import connect_tcp, readuntil2crlfs, FakeLogger, \
    fake_http_connect, FakeMemcache, debug_logger
from osd.objectService.unitTests import FakeObjectRing, FakeContainerRing, FakeAccountRing
from osd.proxyService import server as proxy_server
from osd.accountService import server as account_server
...
from osd.common.middleware import proxy_logging
from osd.common.middleware.acl import parse_acl, format_acl
from osd.common.exceptions import ChunkReadTimeout
from osd.common.constraints import MAX_META_NAME_LENGTH, \
    MAX_META_VALUE_LENGTH, MAX_META_COUNT, MAX_META_OVERALL_SIZE, \
    MAX_FILE_SIZE, MAX_ACCOUNT_NAME_LENGTH, MAX_CONTAINER_NAME_LENGTH, \
    ACCOUNT_LISTING_LIMIT, CONTAINER_LISTING_LIMIT, MAX_OBJECT_NAME_LENGTH, \
    NON_ALLOWED_HEADERS
from osd.common import utils
from osd.common.utils import mkdirs, normalize_timestamp, NullLogger
from osd.common.helper_functions import split_path
from osd.common.wsgi import monkey_patch_mimetools
from osd.proxyService.controllers import base as proxy_base
from osd.proxyService.controllers.base import get_container_memcache_key, \
    get_account_memcache_key, cors_validation
import osd.proxyService.controllers
from osd.common.request_helpers import get_sys_meta_prefix
from osd.common.swob import Request, Response, HTTPUnauthorized, \
    HTTPException
...
def do_setup(the_object_server):
    ...
    osd.objectService.recovery_handler.OSD_DIR = _testdir
    osd.objectService.diskfile.EXPORT_PATH = _testdir
    osd.containerService.transcation_handler.RING_DIR = _testdir
    osd.containerService.container_operations.FS_BASE = _testdir
    mkdirs(_testdir)
    rmtree(_testdir)
    ...
    conf = {'osd_dir': _testdir,
            'mount_check': 'false', 'allowed_headers':
            'content-encoding, x-object-manifest, content-disposition, foo',
            'allow_versions': 'False'}
    prolis = listen(('localhost', 0))
    acclis = listen(('localhost', 0))
    conlis = listen(('localhost', 0))
    objlis = listen(('localhost', 0))
    _test_sockets = \
        (prolis, acclis, conlis, objlis)
    account_ring_path = os.path.join(_testdir, 'account.ring')
    with open(account_ring_path, 'wb') as f:
        f.write(pickle.dumps({'fs_list':['fs1', 'fs2'], \
                     'dir_list':['d0', 'd1', 'd2', 'd3'], \
                     'account_service_list':[{'as_1': {'ip': '127.0.0.1', \
                                               'port': acclis.getsockname()[1]}}], \
                     'comp_list':['x0'], \
                     'comp_account_dist':{"as_1": ["x0"]}}))
    container_ring_path = os.path.join(_testdir, 'container.ring')
    with open(container_ring_path, 'wb') as f:
        f.write(pickle.dumps({'fs_list':['fs1', 'fs2'], \
                     'dir_list':['d0', 'd1', 'd2', 'd3'], \
                     'container_service_list':[{'cs_1': {'ip': '127.0.0.1', \
                                               'port': conlis.getsockname()[1]}}], \
                     'comp_list':['x0'], \
                     'comp_container_dist':{"cs_1": ["x0"]}}))
    object_ring_path = os.path.join(_testdir, 'object.ring')
    with open(object_ring_path, 'wb') as f:
            f.write(pickle.dumps({'fs_list':['fs1', 'fs2'], \
                         'dir_list':['d0', 'd1', 'd2', 'd3'], \
                         'object_service_list':[{'os_1': {'ip': '127.0.0.1', \
                         'port': objlis.getsockname()[1]}}]}))
    prosrv = proxy_server.Application(conf, FakeMemcacheReturnsNone(),
                                      logger=debug_logger('proxy'))
    conf.update({'devices': _testdir})
    accsrv = account_server.AccountController(
        conf, logger=debug_logger('acct1'))
    consrv = container_server.ContainerController(
        conf, logger=debug_logger('cont1'))
    conf.update({'filesystems': _testdir})
    objsrv = the_object_server.ObjectController(
        conf, logger=debug_logger('obj1'))
    _test_servers = \
        (prosrv, accsrv, consrv, objsrv)
    nl = NullLogger()
    logging_prosv = proxy_logging.ProxyLoggingMiddleware(prosrv, conf,
                                                         logger=prosrv.logger)
    prospa = spawn(wsgi.server, prolis, logging_prosv, nl)
    accspa = spawn(wsgi.server, acclis, accsrv, nl)
    conspa = spawn(wsgi.server, conlis, consrv, nl)
    objspa = spawn(wsgi.server, objlis, objsrv, nl)
    _test_coros = \
        (prospa, accspa, conspa, objspa)
    # Create account
    ts = normalize_timestamp(time.time())
    nodes, filesystem, directory, version, comp_no = prosrv.account_ring.get_node('a')
    for node in nodes:
        conn = osd.proxyService.controllers.obj.http_connect(node['ip'],
                                                        node['port'],
                                                        filesystem,
                                                        directory, 'PUT', '/a',
                                                        {'X-Timestamp': ts,
...
@contextmanager
def save_globals():
    orig_http_connect = getattr(osd.proxyService.controllers.base, 'http_connect',
                                None)
    orig_account_info = getattr(osd.proxyService.controllers.Controller,
                                'account_info', None)
    try:
        yield True
    finally:
        osd.proxyService.controllers.Controller.account_info = orig_account_info
        osd.proxyService.controllers.base.http_connect = orig_http_connect
        osd.proxyService.controllers.obj.http_connect = orig_http_connect
        osd.proxyService.controllers.account.http_connect = orig_http_connect
        osd.proxyService.controllers.container.http_connect = orig_http_connect
...
class TestController(unittest.TestCase):
    ...
    def setUp(self):
        self.account_ring = FakeAccountRing('')
        self.container_ring = FakeContainerRing('')
        ...
        app = proxy_server.Application(None, self.memcache,
                                       ...
                                       logger=FakeLogger())
        self.controller = osd.proxyService.controllers.Controller(app)
    ...
    def check_account_info_return(self, nodes, filesystem, directory, global_map_version, comp_no, is_none=False):
        if is_none:
            n, f, d = None, None, None
        else:
            n, f, d, v, c = self.account_ring.get_node(self.account)
        self.assertEqual(f, filesystem)
        self.assertEqual(d, directory)
        self.assertEqual(n, nodes)
        self.assertEqual(v, global_map_version)
        self.assertEqual(c, comp_no)

    def test_account_info_container_count(self):
        with save_globals():
            set_http_connect(200, count=123)
            node, filesystem, directory, count = \
    ...
    def test_make_requests(self):
        with save_globals():
            ...
            self.controller._make_request(
                node, filesystem, directory, 'POST', '/', '', '',
    ...
    def test_account_info_200(self):
        with save_globals():
            ...
            node, filesystem, directory, count = \
                self.controller.account_info(self.account, self.request)
            self.check_account_info_return(node, filesystem, directory)
    ...
    def test_account_info_404(self):
        with save_globals():
            ...
            node, filesystem, directory, count = \
                self.controller.account_info(self.account, self.request)
            self.check_account_info_return(node, filesystem, directory, True)
    ...
    def test_account_info_no_account(self):
        with save_globals():
            ...
            node, filesystem, directory, count = \
                self.controller.account_info(self.account, self.request)
            self.check_account_info_return(node, filesystem, directory, is_none=True)
            self.assertEquals(count, None)

    def check_container_info_return(self, ret, is_none=False):
        if is_none:
            node, filesystem, directory, read_acl, write_acl = None, None, None, None, None
        else:
            node, filesystem, directory, version, comp_no = self.container_ring.get_node(self.account,
                                                             self.container)
            read_acl, write_acl = self.read_acl, self.write_acl
        self.assertEqual(filesystem, ret['filesystem'])
        self.assertEqual(directory, ret['directory'])
        self.assertEqual(node, ret['node'])
        #self.assertEqual(version, ret['version'])
        #self.assertEqual(comp_no, ret['comp_no'])
    ...
    def test_container_info_invalid_account(self):
        ...
        with save_globals():
            osd.proxyService.controllers.Controller.account_info = account_info
...
class TestProxyServer(unittest.TestCase):
    ...
    def test_unhandled_exception(self):
        ...
        app = MyApp(None, FakeMemcache(), account_ring=FakeAccountRing(''),
    ...
    def test_internal_method_request(self):
        baseapp = proxy_server.Application({},
                                           ...
                                           object_ring=FakeObjectRing(''),
                                           account_ring=FakeAccountRing(''), logger=FakeLogger())
    ...
    def test_inexistent_method_request(self):
        baseapp = proxy_server.Application({},
                                           ...
                                           object_ring=FakeObjectRing(''), logger=FakeLogger())
        resp = baseapp.handle_request(
    ...
    def test_negative_content_length(self):
        osd_dir = mkdtemp()
        try:
            baseapp = proxy_server.Application({'osd_dir': osd_dir},
                                               FakeMemcache(), FakeLogger(),
                                               FakeAccountRing(''), FakeContainerRing(''),
                                               FakeObjectRing(''))
            resp = baseapp.handle_request(
            ...
            rmtree(osd_dir, ignore_errors=True)
    ...
    def test_node_timing(self):
        ...
        with mock.patch('osd.proxyService.server.time', lambda: times.pop(0)):
...
class TestObjectController(unittest.TestCase):
    ...
    def test_PUT_message_length_too_large(self):
        osd.proxyService.controllers.obj.MAX_FILE_SIZE = 10
        try:
            prolis = _test_sockets[0]
            sock = connect_tcp(('localhost', prolis.getsockname()[1]))
            fd = sock.makefile()
            ...
            osd.proxyService.controllers.obj.MAX_FILE_SIZE = MAX_FILE_SIZE
    ...
    def test_custom_mime_types_files(self):
        osd_dir = mkdtemp()
        try:
            with open(os.path.join(osd_dir, 'mime.types'), 'w') as fp:
                fp.write('foo/bar foo\n')
            proxy_server.Application({'osd_dir': osd_dir},
                                     FakeMemcache(), FakeLogger(),
                                     FakeAccountRing(''), FakeContainerRing(''),
                                     FakeObjectRing(''))
    ...
    def test_client_timeout(self):
        with save_globals():
            n , f, d, v, c = self.app.account_ring.get_node('account')
            n[0]['ip'] = '127.0.0.1'
            n[0]['port'] = 1
            n, f, d, v, c = self.app.container_ring.get_node('account')
            n[0]['ip'] = '127.0.0.1'
            n[0]['port'] = 1
            n, f, d, v, c = self.app.object_ring.get_node('account')
            n[0]['ip'] = '127.0.0.1'
            n[0]['port'] = 1
    ...
    def test_client_disconnect(self):
        with save_globals():
            ...
            for dev in self.app.account_ring.devs.values():
                dev['ip'] = '127.0.0.1'
                dev['port'] = 1
            self.app.container_ring.get_node('account')
            for dev in self.app.container_ring.devs.values():
                dev['ip'] = '127.0.0.1'
                dev['port'] = 1
            self.app.object_ring.get_node('account')
    ...
    def controller_context(self, req, *args, **kwargs):
        _v, account, container, obj = split_path(req.path, 4, 4, True)
        controller = proxy_server.ObjectController(self.app, account,
                                                   container, obj)
    ...
    def test_chunked_put_bad_version(self):
        # Check bad version
        (prolis, acclis, conlis, objlis) = _test_sockets
        sock = connect_tcp(('localhost', prolis.getsockname()[1]))
        fd = sock.makefile()
...
class TestContainerController(unittest.TestCase):
    "Test osd.proxyService_server.ContainerController"
    ...
    def test_transfer_headers(self):
        ...
        controller = osd.proxyService.controllers.ContainerController(self.app,
                                                                 'a', 'c')
...
class TestAccountController(unittest.TestCase):
    ...
    def test_connection_refused(self):
        n, f, d, v, c = self.app.account_ring.get_node('account')
        n[0]['ip'] = '127.0.0.1'
        n[0]['port'] = 1  # can't connect on this port
        controller = proxy_server.AccountController(self.app, 'account')
        req = Request.blank('/v1/account', environ={'REQUEST_METHOD': 'HEAD'})
    ...
    def test_other_socket_error(self):
        n, f, d, v, c = self.app.account_ring.get_node('account')
        n[0]['ip'] = '127.0.0.1'
        n[0]['port'] = -1  # invalid port number
        controller = proxy_server.AccountController(self.app, 'account')
        req = Request.blank('/v1/account', environ={'REQUEST_METHOD': 'HEAD'})
...
class TestSwiftInfo(unittest.TestCase):
    ...
    def test_registered_defaults(self):
        ...
        self.assertEqual(si['non_allowed_headers'], NON_ALLOWED_HEADERS)
