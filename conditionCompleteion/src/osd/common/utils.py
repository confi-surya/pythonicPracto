...
from osd.common.exceptions import LockTimeout, MessageTimeout
from osd.common.http import is_success, is_redirection, HTTP_NOT_FOUND
#from osd.common.ring.ring import hash_path, ContainerRing
from osd.common.macros import Macros
from osd.common.helper_functions import quote as helper_quote
from osd.common.helper_functions import reiterate as helper_reiterate
from osd.common.helper_functions import split_path as helper_split_path
from osd.common.helper_functions import get_valid_utf8_str as helper_get_valid_utf8_str
from osd.common.swob import HTTPTemporaryRedirect
...
from osd.glCommunicator.communication.osd_exception import FileCorruptedException
...
SWIFT_CONF_FILE = '/opt/HYDRAstor/objectStorage/configFiles/osd.conf'
RECOVERY_FILE_PATH = '/var/run/osd/recovery'
LOG_CONF_FILE = '/opt/HYDRAstor/objectStorage/configFiles/logging.conf'
DEFAULT_LOG_DIR = "/var/log/osd"
OSD_DIR = "/opt/HYDRAstor/objectStorage/configFiles"

from logging_handler import MyRotatingFileHandler, MyBaseRotatingHandler

def get_host_from_map(account, container, global_map, osd_dir, logger):            
    """                                                                
    :param account: account name                                       
    :param container: container name                                   
    :param container: global_map                                       
    :return:list of ip port as a dict.                                 
    """                                                                
    try:
        #moved here as using interfaces from osd_utils have cyclic 
        #dependency which could be solved through thi 
        from osd.common.ring.ring import ContainerRing
  
        container_ring = ContainerRing(osd_dir, logger)           
        comp_no = container_ring.get_component(account, container)
        logger.debug("Comp_no: %s "% comp_no)
        container_obj = global_map['container-server'][0][comp_no]     
        data = {}                                                      
        data['ip'] = container_obj.get_ip()                            
        data['port'] = container_obj.get_port()                        
    except Exception:
        logger.debug("get_host_from map exception: %s "% comp_no)
        return ''
    if (not data['ip']) or (not data['port']):
        logger.debug("empty ip, port : %s" %(data))
        return ''

    logger.debug('%(ip)s:%(port)s' % data)
    return '%(ip)s:%(port)s' % data

#The req parameter used in this function is only used to differentiate the 
#return of this function, 
#If req valid, return Value error exception
#If Req not valid, system.exit()
def get_global_map(service_id, logger, req=None, time_=1):
    """Get Global Map with return http response"""
    request_obj = Req(logger)#Here if timeout=0 have no any specific purpose, just use Req(logger)
    retry_count = 0
    while retry_count < 3:
        retry_count += 1
        try:
            gl_info_obj = request_obj.get_gl_info()
        except FileCorruptedException, ex:
            logger.error(ex)
            sleep(time_)
            continue
        logger.debug("Gl map  retry count: %s, connecting with: %s" \
            %(retry_count, gl_info_obj))
        conn_obj = request_obj.connector(IoType.EVENTIO, gl_info_obj)
        if not conn_obj:
            logger.error("Failed to connect with: %s" % gl_info_obj)
            sleep(time_)
            continue
        global_map_obj = request_obj.get_global_map(service_id, conn_obj)
        if global_map_obj.status.get_status_code() == Resp.SUCCESS:
            return global_map_obj.message.map(), \
                global_map_obj.message.version(), conn_obj
        logger.error("While reading gl map: status code: %s, status message: %s" \
            %(global_map_obj.status.get_status_code(), global_map_obj.status.get_status_message()))
        sleep(time_)

    if req:
        raise ValueError('No Map could be obtained')
    sys.exit(130)


def get_global_map_version(service_id, logger):
    """Get Global Map with return http response"""
    request_obj = Req(logger)#Here if timeout=0 have no any specific purpose, just use Req(logger)
    retry_count = 0
    while retry_count < 3:
        ...
        global_map_obj = request_obj.get_global_map(service_id, conn_obj)
        if global_map_obj.status.get_status_code() == Resp.SUCCESS:
            return global_map_obj.message.version()
        logger.error("While reading gl map: status code: %s, status message: %s" \
            %(global_map_obj.status.get_status_code(), global_map_obj.status.get_status_message()))
...
def get_valid_utf8_str(str_or_unicode):
	return helper_get_valid_utf8_str(str_or_unicode)

def quote(value, safe='/'):
	return helper_quote(value, safe)

def split_path(path, minsegs=1, maxsegs=None, rest_with_last=False):
	return helper_split_path(path, minsegs, maxsegs, rest_with_last)

def reiterate(iterable):
	return helper_reiterate(iterable)
...
class InvalidLogFileConfigError(ValueError):

    def __str__(self):
        return "[logging-info]: some necessary options or sections "\
            "are missing from %s " % LOG_CONF_FILE


class InvalidHashPathConfigError(ValueError):

    def __str__(self):
        return "[osd-hash]: both osd_hash_path_suffix and " \
            "osd_hash_path_prefix are missing from %s" % SWIFT_CONF_FILE



def load_log_config(section='server'):
    
    conf = {}
    log_conf = ConfigParser()
    if log_conf.read(LOG_CONF_FILE):
        try:
            conf['log_file_dir'] = log_conf.get(section,'log_dir')
            conf['log_file_name'] = log_conf.get(section,'log_file')
            conf['log_file_size'] = log_conf.get(section,'log_file_size')
            conf['log_backup_count'] = log_conf.get(section,'log_backup_count')
        except (NoSectionError, NoOptionError):
            raise InvalidLogFileConfigError()
    if not conf:
        raise InvalidLogFileConfigError()
    return conf
...
def validate_hash_conf():
    HASH_PATH_SUFFIX = Macros.getValue("HASH_PATH_SUFFIX")
    HASH_PATH_PREFIX = Macros.getValue("HASH_PATH_PREFIX")
    if not HASH_PATH_SUFFIX and not HASH_PATH_PREFIX:
        hash_conf = ConfigParser()
        if hash_conf.read(SWIFT_CONF_FILE):
            try:
                Macros.setValue("HASH_PATH_SUFFIX", hash_conf.get('osd-hash',
                                                 'osd_hash_path_suffix'))
            except (NoSectionError, NoOptionError):
                pass
            try:
                Macros.setValue("HASH_PATH_PREFIX", hash_conf.get('osd-hash',
                                                 'osd_hash_path_prefix'))
            except (NoSectionError, NoOptionError):
                pass
        if not Macros.getValue("HASH_PATH_SUFFIX") \
		and not Macros.getValue("HASH_PATH_PREFIX"):
...
# Used by get_osd_info and register_osd_info to store information about
# the osd cluster.
_osd_info = {}
_osd_admin_info = {}


def get_osd_info(admin=False, disallowed_sections=None):
    """
    Returns information about the osd cluster that has been previously
    registered with the register_osd_info call.
    ...
    :returns: dictionary of information about the osd cluster.
    """
    disallowed_sections = disallowed_sections or []
    info = {}
    for section in _osd_info:
        if section in disallowed_sections:
            continue
        info[section] = dict(_osd_info[section].items())
    if admin:
        info['admin'] = dict(_osd_admin_info)
        info['admin']['disallowed_sections'] = list(disallowed_sections)
    return info


def register_osd_info(name='osd', admin=False, **kwargs):
    """
    Registers information about the osd cluster to be retrieved with calls
    to get_osd_info.
    ...
    if admin:
        dict_to_use = _osd_admin_info
    else:
        dict_to_use = _osd_info
...
def convert_to_unsigned_long(float_val):
    """
    Convert a float value to ctype unsigned long value
    :param float_val: python float value
    :returns: ctype unsigned long value
    """
...
def timing_stats(**dec_kwargs):
    ...
    osd's wsgi server controllers, based on response code.
    """
...
class SwiftLogFormatter(logging.Formatter):
    ...
    def format(self, record):
        if not hasattr(record, 'server'):
            # Catch log messages that were not initiated by osd
...
def get_logger(conf, name=None, log_to_console=False, log_route=None, section='server',
               fmt="%(asctime)s : %(levelname)s: %(server)s :%(filename)s: %(lineno)s: %(message)s"):
    """
    ...
    **Log config and defaults**::
    ...
    if not conf:
        conf = {}
    if name is None:
        name = conf.get('log_name', 'osd')
    if not log_route:
        log_route = name
    
    log_conf = load_log_config(section)
    log_file_path = log_conf.get('log_file_dir', DEFAULT_LOG_DIR)
    log_file_name = log_conf.get('log_file_name', 'swift.log')
    log_file_size = int(log_conf.get('log_file_size', 50*1024*1024))
    log_backup_count = int(log_conf.get('log_backup_count', 6))
    

    create_log_file(log_file_path, log_file_name)
    ...
    '''
    ##removing syslog handler
    
    # facility for this logger will be set by last call wins
    facility = getattr(SysLogHandler, conf.get('log_facility', 'LOG_LOCAL6'),
                       SysLogHandler.LOG_LOCAL6)
    ...
    '''
    handler = MyRotatingFileHandler(log_file, maxBytes = log_file_size, backupCount = log_backup_count)
...
'''
#NOTE: moved the implementation to common.ring.ring
...
def get_service_id(server):
    ...
    if server  in SERVERS:
        ...
        return host_name + "_" + server +"-" + "server"
...
def cache_from_env(env):
    ...
    :returns: osd.common.memcached.MemcacheRing from environment
    """
...
def remove_tmp_file_if_exist(dir_name, file_name):
    """
    Deletes tmp file if exist in case of object PUT for previous
    failed write

    :param dir_name: directory under which files will be created
    :param file_name: name the file to be created
    """
    file_path = os.path.join(dir_name, file_name)
    if os.path.exists(file_path):
        remove_file(file_path)

def create_tmp_file(dir_name,file_name):
    """Create and open file
    
    :param dir_name: directory under which files will be created
    :param file_name: name the file to be created
    """
    remove_tmp_file_if_exist(dir_name, file_name)
    flags = os.O_RDWR | os.O_CREAT | os.O_EXCL
    file = os.path.join(dir_name,file_name)
    try:
        fd = os.open(file,flags,0o600)
        return (fd, os.path.abspath(file))
    except OSError :
        pass
...
def loggit(server_name):
    '''Decorator to log the output of function'''
    def decorating_func(func):
        method = func.func_name

        @functools.wraps(func)
        def wrapper(ctrl, *args, **kwargs):
            ctrl.logger.debug("Starting %s %r method, path: %s, headers: %s, \
                transaction-id: %s" \
                    % (server_name, func.__name__, args[0].path, \
                        args[0].headers, args[0].headers.get('x-trans-id', '-')))
            result = func(ctrl, *args, **kwargs)
            ctrl.logger.debug("Completed %s %r method, result: %s, headers: %s, \
                status: %s, transaction-id: %s" \
                    % (server_name, func.__name__, result, result.headers, \
                        result.status, args[0].headers.get('x-trans-id', '-')))
            return result
        return wrapper
    return decorating_func
