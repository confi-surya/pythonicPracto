...
from osd.common import utils, constraints
from osd.common.swob import Request
from osd.common.utils import capture_stdio, disable_fallocate, \
    drop_privileges, get_logger, NullLogger, config_true_value, \
    validate_configuration, get_hub, config_auto_int_value

from osd.common.helper_functions import CloseableChain
...
class PipelineWrapper(object):
    ...
    def _format_for_display(self, ctx):
        if ctx.entry_point_name:
            ...
            #
            #    use = egg:osd#healthcheck
            #
            # you have something like this:
            #
            #    paste.filter_factory = \
            #        osd.common.middleware.healthcheck:filter_factory
...
def run_server(conf, logger, sock, global_conf=None):
    ...
    try:
        ...
        func()
        wsgi.server(sock, app, NullLogger(), custom_pool=pool, capitalize_response_headers=False)
    except socket.error as err:
...
def run_wsgi(conf_path, app_section, *args, **kwargs):
    ...
    sock = get_socket(conf, default_port=kwargs.get('default_port', 61005))
    # remaining tasks should not require elevated privileges
    drop_privileges(conf.get('user', 'swift'))

    # Ensure the configuration and application can be loaded before proceeding.
    global_conf = {'log_name': log_name}
    global_conf.update({'llport':conf['llport']})
    if 'global_conf_callback' in kwargs:
        kwargs['global_conf_callback'](conf, global_conf)

    clean_journal = "False"
    if kwargs.get("clean_journal"):
        clean_journal = "True"

    global_conf.update({'clean_journal': clean_journal})
    ...
    # deveshg: explicitly set worker_count to 0. HYD-5836
    worker_count = 0	#NOTE: we cannot allow multiple process.
    if worker_count == 0:
