...
from osd.common.memcached import MemcacheRing


class MemcacheMiddleware(object):
    """
    Caching middleware that manages caching in osd.
    """
    ...
    def __init__(self, app, conf):
        ...
        if (not self.memcache_servers
                ...
                or max_conns <= 0):
            path = os.path.join(conf.get('osd_dir', '/opt/HYDRAstor/objectStorage/configFiles'),
                                'memcache.conf')
        ...
        if not self.memcache_servers:
            self.memcache_servers = '127.0.0.1:61010'
        if max_conns <= 0:
            max_conns = 2
        if serialization_format is None:
