...
* remote_addr is the contents of the REMOTE_ADDR environment variable, while
  client_ip is osd's best guess at the end-user IP, extracted variously
...
needed. With the doubled proxy logs, any consumer/processor of osd's proxy
...
from osd.common.utils import (get_logger, get_remote_client,
                                config_true_value,
                                InputProxy, list_from_csv)
from osd.common.helper_functions import get_valid_utf8_str
from osd.common.constraints import MAX_HEADER_SIZE
...
class ProxyLoggingMiddleware(object):
    """
    Middleware that logs Swift proxy requests in the osd log format.
    """
    ...
    def __init__(self, app, conf, logger=None):
        ...
        self.access_logger = logger or get_logger(access_log_conf,
                                                  log_route='proxy-access', section='proxy-access',
