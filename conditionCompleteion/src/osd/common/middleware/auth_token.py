...
class AuthProtocol(object):
    ...
    def __init__(self, app, conf):
        ...
        self.LOG.debug('Starting keystone auth_token middleware')
        self.LOG.warning(
        ...
        self.LOG.debug('Using %s as cache directory for signing certificate',
    ...
    def _choose_api_version(self):
        ...
        if self._conf_get('auth_version'):
            ...
            self.LOG.debug('Auth Token proceeding with requested %s apis',
                          version_to_use)
        else:
            version_to_use = None
            ...
            if versions_supported_by_server:
                for version in LIST_OF_VERSIONS_TO_ATTEMPT:
                    if version in versions_supported_by_server:
                        ...
                        break
            if version_to_use:
                self.LOG.debug('Auth Token confirmed use of %s apis',
                              version_to_use)
            else:
                self.LOG.error(
    ...
    def __call__(self, env, start_response):
        ...
        try:
            ...
            if self.delay_auth_decision:
                ...
                self.LOG.error('Invalid user token - rejecting request')
    ...
    def _validate_user_token(self, user_token, env, retry=True):
        ...
        try:
            ...
            self.LOG.error('Token validation failure.', exc_info=True)
            if token_id:
    ...
    def _confirm_token_bind(self, data, env):
        ...
        if not bind:
            if permissive:
                ...
                self.LOG.error('No bind information present in token.')
                self._invalid_user_token()
        ...
        if name and name not in bind:
            self.LOG.error('Named bind mode %s not in bind information', name)
            self._invalid_user_token()

        for bind_type, identifier in six.iteritems(bind):
            if bind_type == BIND_MODE.KERBEROS:
                if not env.get('AUTH_TYPE', '').lower() == 'negotiate':
                    self.LOG.error('Kerberos credentials required and '
                                  'not present.')
                    self._invalid_user_token()

                if not env.get('REMOTE_USER') == identifier:
                    self.LOG.error('Kerberos credentials do not match '
                                  'those in bind.')
                    self._invalid_user_token()
                ...
                self.LOG.error('Couldn`t verify unknown bind: %(bind_type)s: '
                              '%(identifier)s.',
                              {'bind_type': bind_type,
                               'identifier': identifier})
                self._invalid_user_token()
    ...
    def verify_uuid_token(self, user_token, retry=True):
        ...
        if response.status_code == 401:
            ...
            self.LOG.error('Bad response code while validating token: %s',
                           response.status_code)
        if retry:
            self.LOG.debug('Retrying validation')
...
class TokenCache(object):
    ...
    def _cache_get(self, token_id):
        ...
        if cached == self._INVALID_INDICATOR:
            self.LOG.error('Cached Token is marked unauthorized')
            raise InvalidUserToken('Token authorization failed')
        ...
        if utcnow < expires:
            ...
            self.LOG.error('Cached Token seems expired')
            raise InvalidUserToken('Token authorization failed')
