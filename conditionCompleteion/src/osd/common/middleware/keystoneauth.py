...
from osd.common import utils as osd_utils
from osd.common.middleware import acl as osd_acl
from osd.common.swob import HTTPNotFound, HTTPForbidden, HTTPUnauthorized
from osd.common.utils import register_osd_info
...
class KeystoneAuth(object):
    ...
    And add a osd authorization filter section, such as::

        [filter:keystoneauth]
        use = egg:osd#keystoneauth
    ...
    def __call__(self, environ, start_response):
        ...
        environ['swift.clean_acl'] = osd_acl.clean_acl
    ...
    def authorize_for_bandwidth_limit(self, req):
        """Check reseller admin authorization for bandwidth limit feature
        during post metadata on account

        :return: True if X-Account-Meta-Max-Read-Bandwidth or
                 X-Account-Meta-Max-Read-Bandwidth exists in request headers.
                 False otherwise.

        """
        if 'X-Account-Meta-Max-Read-Bandwidth' in req.headers or \
            'X-Account-Meta-Max-Write-Bandwidth' in req.headers:
            return True
        else:
            return False
    ...
    def authorize(self, req):
        ...
        if "_member_" in user_roles and env.get('swift.source', '').startswith('S3'):
    ...
    def _authorize_unconfirmed_identity(self, req, obj, referrers, roles):
        ...
        if osd_acl.referrer_allowed(req.referer, referrers):
...
def filter_factory(global_conf, **local_conf):
    ...
    register_osd_info('keystoneauth')
