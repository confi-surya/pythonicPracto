...
from osd.common.exceptions import ListingIterError
from osd.common.swob import Request, HTTPBadRequest, HTTPServerError, \
    HTTPMethodNotAllowed, HTTPRequestEntityTooLarge, HTTPLengthRequired, \
    HTTPOk, HTTPPreconditionFailed, HTTPException, HTTPNotFound, \
    HTTPUnauthorized, HTTPRequestedRangeNotSatisfiable, Response
from osd.common.utils import json, get_logger, config_true_value, \
    get_valid_utf8_str, override_bytes_from_content_type, split_path, \
    register_osd_info, RateLimitedIterator, quote
from osd.common.request_helpers import SegmentedIterable, \
    closing_if_possible, close_if_possible
from osd.common.constraints import check_utf8, MAX_BUFFERED_SLO_SEGMENTS
from osd.common.http import HTTP_NOT_FOUND, HTTP_UNAUTHORIZED, is_success
from osd.common.wsgi import WSGIContext, make_subrequest
#from osd.common.middleware.bulk import get_response_body, \
#    ACCEPTABLE_FORMATS, Bulk
from osd.common.bulk_delete import get_response_body, \
    ACCEPTABLE_FORMATS 
...
#        self.bulk_deleter = Bulk(app, {}, logger=self.logger)
    ...
    def handle_multipart_put(self, req, start_response):
        ...
        try:
            ...
            self.logger.info("Received manifest file: %s for upload" %(req.path))
        except ValueError:
    ...
    def get_segments_to_delete_iter(self, req):
        ...
        while segments:
            if len(segments) > MAX_BUFFERED_SLO_SEGMENTS:
                raise HTTPBadRequest(
                    'Too many buffered slo segments to delete.') #Handle this Raise
    ...
    def handle_multipart_delete(self, req):
        ...
        """
        self.logger.info("Received Delete for manifest file: %s" %(req.path))
        self.objs_to_delete = []
        self.multipart_delete_helper(req)
        self.logger.info("Objects to delete in SLO: %s" %(self.objs_to_delete))
        return self.objs_to_delete
...
#	resp = ContainerController.bulk_delete_helper(req, objects_to_delete)
#        if out_content_type:
#            resp.content_type = out_content_type
#        return resp

    def multipart_delete_helper(self, req):
        for obj_to_delete in self.get_segments_to_delete_iter(req):
            self.logger.debug("Object segment for DELETE: %s" %(obj_to_delete))
            obj_name = obj_to_delete['name']
            self.logger.debug("Name of Object to DELETE: %s" %(obj_name)) 
            if not obj_name:
                continue
            else:
                self.objs_to_delete.append(obj_name)
