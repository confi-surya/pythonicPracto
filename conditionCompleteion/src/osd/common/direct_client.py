...
from osd.common.exceptions import ClientException
from osd.common.utils import normalize_timestamp, FileLikeIter
from osd.common.http import HTTP_NO_CONTENT, HTTP_INSUFFICIENT_STORAGE, \
    is_success, is_server_error
from osd.common.swob import HeaderKeyDict
from osd.common.utils import quote
...
def direct_put_account(node, part, account, conn_timeout=5,
                       response_timeout=15, headers=None):
    """
    Request account put directly from the account server.
    ...
    if not is_success(resp.status):
        raise ClientException(
            'Account server %s:%s direct PUT %s gave status %s' %
            (node['ip'], node['port'],
             repr('/%s/%s%s' % (node['device'], part, path)),
             resp.status),
            http_host=node['ip'], http_port=node['port'],
...
def direct_head_account(node, part, account, conn_timeout=5,
                        response_timeout=15):
    """
    Request account head directly from the account server.
    ...
    if not is_success(resp.status):
        raise ClientException(
            'Account server %s:%s direct HEAD %s gave status %s' %
            (node['ip'], node['port'],
             repr('/%s/%s%s' % (node['device'], part, path)),
             resp.status),
            http_host=node['ip'], http_port=node['port'],
...
def direct_post_account(node, part, account, headers,
                        conn_timeout=5, response_timeout=15):
    """
    Request account post directly from the account server.
    ...
    if not is_success(resp.status):
        raise ClientException(
            'Account server %s:%s direct POST %s gave status %s' %
            (node['ip'], node['port'],
             repr('/%s/%s%s' % (node['device'], part, path)),
             resp.status),
            http_host=node['ip'], http_port=node['port'],
...
def direct_delete_account_metadata(node, part, account, headers,
                        conn_timeout=5, response_timeout=15):
    """
...
def direct_delete_account(node, part, account, conn_timeout=5,
                          response_timeout=15, headers=None):
    """
    Request account delete directly from the account server.
...
def direct_post_container(node, part, account, container, headers,
    conn_timeout=5, response_timeout=15):
    """
    Request container post directly from the container server.
    ...
    if not is_success(resp.status):
        raise ClientException(
            'Container server %s:%s direct POST %s gave status %s' %
            (node['ip'], node['port'],
            repr('/%s/%s%s' % (node['device'], part, path)), resp.status),
            http_host=node['ip'], http_port=node['port'],
            http_device=node['device'], http_status=resp.status,
