...
from osd.common.utils import search_tree, remove_file, write_file, get_logger

SWIFT_DIR = '/opt/HYDRAstor/objectStorage/configFiles'
OSD_DIR = '/export/.osd_meta_config'
RUN_DIR = '/var/run/osd'
OSD_SCRIPTS_PATH = '/opt/HYDRAstor/objectStorage/scripts'

# auth-server has been removed from ALL_SERVERS, start it explicitly
ALL_SERVERS = ['account-server', 'container-server', 'object-server',
               'proxy-server', 'account-updater' ]
MAIN_SERVERS = ['account-server', 'container-server', 'object-server',
               'proxy-server']
...
class Manager(object):
    ...
    def __init__(self, servers, run_dir=RUN_DIR):
        ...
        for server in servers:
            if server == 'all':
                server_names.extend(ALL_SERVERS)
            elif server == 'main':
                server_names.extend(MAIN_SERVERS)
            elif server == 'rest':
                server_names.extend(REST_SERVERS)
            elif '*' in server:
                # convert glob to regex
                server_names.extend([s for s in ALL_SERVERS if
                                     re.match(server.replace('*', '.*'), s)])
            else:
                server_names.append(server)
        self.servers = []
        for name in server_names:
            self.servers.append(Server(name, run_dir))
...
class Server(object):
    ...
    def __init__(self, server, run_dir=RUN_DIR):
        ...
        self.cmd = os.path.join(OSD_SCRIPTS_PATH, 'osd-%s' % server)
        self.procs = []
    ...
    def get_pid_file_name(self, conf_file):
        ...
        if self.type == 'proxy':
            return conf_file.replace(
                os.path.normpath(OSD_DIR), self.run_dir, 1).replace(
    ...
    def get_conf_file_name(self, pid_file):
        ...
        if self.server in STANDALONE_SERVERS:
            ...
            return pid_file.replace(
                os.path.normpath(self.run_dir), OSD_DIR, 1).replace(
    ...
    def spawn(self, conf_file, once=False, wait=True, daemon=True, **kwargs):
        ...
        """
        args = [self.cmd, conf_file, '-l %s'%kwargs['llport']]
        if kwargs.has_key('clean_journal'):
            args.append("--clean_journal")
        if once:
            args.append('once')
        if not daemon:
    ...
    def wait(self, **kwargs):
        ...
        for proc in self.procs:
            ...
            script_logger.info("output of start of service [%s]: %s " %(proc.pid, output))
    ...
    def interact(self, **kwargs):
        ...
        for proc in self.procs:
            ...
            script_logger.info("PID: %s. output : %s | ERROR : %s" %(proc.pid, out, err))
            if proc.returncode:
                status += 1
        return status
    ...
    def launch(self, **kwargs):
        ...
        if self.server == "container-server":
            proc = subprocess.Popen("pkill container", \
        ...
        if self.server == "container-server" and ret == 0:
            pids = {}
        ...
        for pid, pid_file in pids.items():
            ...
            if conf_file in conf_files:
                ...
                script_logger.info("%s running (%s - %s)" %(self.server, pid, pid_file))

        if already_started:
            print _("%s already started...") % self.server
            script_logger.info("%s already started..." %self.server)
            return []
        ...
        for conf_file in conf_files:
            if kwargs.get('once'):
                ...
                script_logger.info("Running %s once" %self.server)
            else:
                msg = _('Starting %s') % self.server
            print '%s...(%s)' % (msg, conf_file)
            script_logger.info("%s...(%s)" %(msg, conf_file))
            try:
                ...
                if e.errno == errno.ENOENT:
                    ...
                    script_logger.error("%s does not exist" %self.cmd)
                    break
                else:
                    raise
            pids[pid] = conf_file
    ...
    def stop(self, **kwargs):
        ...
        """
        resp = self.kill_running_pids(**kwargs)

        if self.server == "container-server":
            os.system("ps -ef|awk \
                '/\/opt\/HYDRAstor\/objectStorage\/scripts\/osd-container-server/ \
