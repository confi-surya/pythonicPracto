...
from osd.common.http import HTTP_NOT_FOUND
from osd.common.swob import Request
from osd.common.wsgi import loadapp
...
class InternalClient(object):
    """
    An internal client that uses a osd proxy app to make requests to Swift.
    ...
    def make_path(self, account, container=None, obj=None):
        """
        Returns a osd path for a request quoting and utf-8 encoding the path
        parts as need be.

        :param account: osd account
...
def get_auth(url, user, key, auth_version='1.0', **kwargs):
    if auth_version != '1.0':
        exit('ERROR: osdclient missing, only auth v1.0 supported')
...
class SimpleClient(object):
    """
    Simple client that is used in bin/osd-dispersion-* and container sync
    """
    ...
    def get_account(self, *args, **kwargs):
        # Used in osd-dispersion-populate
